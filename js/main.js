...
function inicio() {
	...
	for (let i = 0; i < 10; i++) {
		...
		moneda.vel = Phaser.Math.Between(2, 5)/15
	...
	if (!elegirCaja && empezoJuego && !pantFinal) {
		...
		for (let a = 0; a < CAJAS.length; a++) {
			...
			else
			{
				...
				CAJAS[c].velCaja = Phaser.Math.Between(3, 6)/10;
